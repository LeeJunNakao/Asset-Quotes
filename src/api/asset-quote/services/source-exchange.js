...
module.exports = {
  searchB3: async ({ assetCodes, date, currency }) => {
    ...
    const parsedResult = await strapi
      .service("api::asset-quote.source-exchange")
      .parseB3Result({ result, date });
  ...
  parseB3Result: async ({ result, date }) => {
    ...
    const parseAdapter =
      ...
      ([key, quotes]) =>
        [key, fn(quotes)];
    const checkAssetValidity = ([key, quotes]) => [
      key,
      quotes.length === 0 ? new InexistentAsset(key) : quotes,
    ];
  ...
  searchCrypto: async ({ assetCodes, date, currency }) => {
    const promises = assetCodes.map(async (code) => {
      const response = await axios.get(
        `https://min-api.cryptocompare.com/data/v2/histoday?fsym=${code}&tsym=${currency}&toTs=${dateToSeconds(
          date
        )}&limit=1`
      );

      if (response.data.Response === "Error")
        return [code, new InexistentAsset(code)];
